...
{
/*!
  \~english A source that provides a continous Triangle wave with the set frequency.

  The **harmonics** constructor parameter determines the quality of the waveform.
  \~hungarian Egy megadott frekvenciájú háromszöges hullámot szolgáltató forrás.

  A **harmonics** konstruktor paraméter szabályozza a hullámjel minőségét.
*/
template <typename T>
class TriangleGenerator : public Mixer::iWaveformGenerator<T>, public Mixer::AudioSource
{
private:
private:
    float frequency;

...
/*!
  ...
  \~hungarian Egy megadott frekvenciájú fűrészfogas hullámot szolgáltató forrás.

  A **harmonics** konstruktor paraméter szabályozza a hullámjel minőségét.
*/
template <typename T>
class SawtoothGenerator : public Mixer::iWaveformGenerator<T>, public Mixer::AudioSource
{
  private:
    float frequency;
  ...
  public:
    ...
    /*!
        ...
        */
        SawtoothGenerator(const float &frequency, const unsigned int &harmonics, const unsigned int &channels, const unsigned int &sampleRate);
    };
};